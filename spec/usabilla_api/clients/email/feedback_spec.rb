...
module UsabillaApi
  describe UsabillaApi::Clients::Email::Feedback do

    let(:params) { { 'id' => '12345' } }
    let(:response) { UsabillaApi::Clients::Email::Feedback.retrieve(params) }
    ...
    describe '#retrieve' do
      before :each do
        stub_request(:get, uri_stem(UsabillaApi.configuration.uri_email_feedback, params['id'])).to_return(:body => feedback_json)
      end

      context 'when we call email feedback retrieve' do
        it { expect(response.items.first).to be_an_instance_of UsabillaApi::Models::Email::Feedback::Item }
      end
